...
const CardDoctor = ({
  ...
  // Start with empty states (safe for SSR)
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedMap = localStorage.getItem('activeMap');
      const savedDoctors = localStorage.getItem('doctorName');

      if (savedMap) setActiveMap(JSON.parse(savedMap));
      if (savedDoctors) setDoctorNameList(JSON.parse(savedDoctors));
    }
  }, []);
  ...
  const toggleActive = (id, name) => {
    ...
    // Add doctor name to list if not already present
    setDoctorNameList(prev => {
      const exists = prev.find(doc => doc.name === name);
      if (exists) return prev.filter(doc => doc.name !== name); // remove if already there
      return [...prev, { name }];
    });
  };
    ...
    <div className="bg-white shadow rounded-lg">
        ...
        {(doctorName || doctorPost || speciality || experience || iconClass) && (
          <div className="text-gray">
            ...
            <div className={`icon-heart group hover:bg-primary mt-2 ${activeMap[id] ? 'active bg-primary text-white' : 'text-primary bg-transparent'}`}>
              <button
                onClick={() => toggleActive(id, doctorName)}
      ...
      {(btnOutlineLink || btnSecondaryLink) && (
        <div className="bg-primary/10 py-4 px-5 flex justify-between gap-5 rounded-bl-2xl rounded-br-2xl">
          {btnOutline && <a href={btnOutlineLink} className={`${btnOutline}`}>{outlineCtaText}</a>}
          {btnSecondary && <a href={btnSecondaryLink} className={`${btnSecondary}`}>{secondaryCtaText}</a>}
        </div>
      )}
    </div>
  );
